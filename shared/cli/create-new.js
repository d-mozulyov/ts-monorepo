...
async function selectOperation() {
  ...
  const selectedProject = projectOptions[projectIndex];

  // If Electron is selected, show an additional menu
  if (selectedProject === 'Electron') {
    const electronFrameworks = ['Solid', 'React', 'Vue', 'Svelte', 'Vanilla'];
    const frameworkIndex = await createInteractiveMenu('Select a framework for Electron:', electronFrameworks);
    selectedProject = `Electron ${electronFrameworks[frameworkIndex]}`;
  }
